...
**이 레포는 아직 디지몬 앱이 아닙니다.** 솔직하게 적어둡니다.

| 영역 | 상태 |
|---|---|
| 토큰 집계 · 메뉴바 · 설정 | 동작함 (upstream 기능 그대로) |
| 수집 · 육성 · 진화 시스템 | 동작하지만 **아직 포켓몬 데이터** (PokéAPI 사용) |
| 디지몬 데이터 · 죠그레스 · 아머 진화 | **설계만 완료, 미구현** |

지금 빌드해서 실행하면 포켓몬이 나옵니다. 디지몬 전환(Phase 1)은 아직 시작하지 않았습니다.

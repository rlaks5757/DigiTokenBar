...
| 상세 패널 | 동작함 — 세대 · 속성 · 형태 · 소개문 · 필살기 (Wikimon 출처) |

지금 빌드해서 실행하면 디지몬이 부화합니다. 죠그레스 · 아머 진화는 아직입니다.
...
python 테스트 → swift 테스트(1163개) → 로직 코어 커버리지(≥75%) 순으로 검사합니다.
...
번들된 디지몬 설정 데이터(`Resources/digimon_details.json`)는 [Wikimon](https://wikimon.net) 에서
가져왔으며 **CC BY-SA 3.0** 을 따릅니다. 출처와 라이선스는 파일 안에도 기록해 두었습니다.

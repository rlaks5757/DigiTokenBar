...
doc_check() {
  ...
  if [[ -n "$last_tag" ]]; then
    ui_changed=$(git diff --name-only "$last_tag"..HEAD -- 'Sources/DigiTokenBar/UI/' 2>/dev/null)
    ...
    ui_feats=$(git log "$last_tag"..HEAD --format='%s' -- 'Sources/DigiTokenBar/UI/' 2>/dev/null \
...
echo "=== DigiTokenBar 릴리스 $PREV → $VERSION ==="
...
SIGN_IDENTITY="${CODESIGN_IDENTITY:-DigiTokenBar Local}"
# "DigiTokenBar Local" leaf SHA-1 — 설치본 DR 의 certificate leaf pin. 인증서를 재생성/교체하면 갱신 필요.
...
rm -f build/DigiTokenBar.zip
ditto -c -k --keepParent build/DigiTokenBar.app build/DigiTokenBar.zip
BUILT=$(/usr/libexec/PlistBuddy -c "Print CFBundleShortVersionString" build/DigiTokenBar.app/Contents/Info.plist)
...
gh release create "v$VERSION" build/DigiTokenBar.zip --repo "$REPO" \
  --title "DigiTokenBar v$VERSION" --target main --notes-file "$NOTES_FILE"

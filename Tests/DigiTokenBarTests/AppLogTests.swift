...
final class AppLogTests: XCTestCase {
    ...
    func testAppLogWriteAndFlushRoutesThroughBackend() throws {
        let source = try String(contentsOf: repositoryFile("Sources/DigiTokenBar/Core/AppLog.swift"))
    ...
    func testMarkCleanUsesWriteAndFlushNotWriteAlone() throws {
        let source = try String(contentsOf: repositoryFile("Sources/DigiTokenBar/Core/CrashReporter.swift"))
        let body = try XCTUnwrap(
    ...
    func testDuplicateYieldUsesWriteAndFlushBeforeTerminate() throws {
        let source = try String(contentsOf: repositoryFile("Sources/DigiTokenBar/DigiTokenBarApp.swift"))
        let body = try XCTUnwrap(
